...
in the appspec.yml file I define the files and the destination. The source points to my artifact files (which are stored in s3) and the destination is the folder I would like them to be copied to (inside the EC2). It is in the appspec file we can tap into the various lifecycle events that occur during deployment.
